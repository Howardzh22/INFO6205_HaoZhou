    ...
    private void benchmarkThreeSum(final String description, final Consumer<int[]> function, int n, final TimeLogger[] timeLoggers) {
        ...
        if (description.equals("ThreeSumQuadratic") && n<=4000 )
            try{
                ...
                double time = target.lap();
                timeLoggersQuadratic[0].log(time,n);
                timeLoggersQuadratic[1].log(time,n);
        ...
        if (description.equals("ThreeSumQuadrithmic") && n<=4000 )
            try{
                ...
                function.accept(supplier.get());
                double time = target.lap();
                timeLoggersQuadrithmic[0].log(time,n);
                timeLoggersQuadrithmic[1].log(time,n);
        ...
        if (description.equals("ThreeSumCubic") && n<=4000 )
            try{
                ...
                function.accept(supplier.get());
                double time = target.lap();
                timeLoggersCubic[0].log(time,n);
                timeLoggersCubic[1].log(time,n);
